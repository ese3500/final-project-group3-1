...
def get_image(buf):
    ...
    im = Image.fromarray(np.uint8(cm.jet(frame)*255)).convert('RGB').rotate(180)#.transpose(Image.FLIP_LEFT_RIGHT)
...
class StreamingHandler(server.BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            ...
            print("Unknown confidence level: " + str(label[3]*100) + "%")
            print()
