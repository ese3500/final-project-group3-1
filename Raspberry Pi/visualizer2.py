...
<body>
    ...
    <center>
        ...
        <br><br>
        <div width="20%" height="5%"><button id = "classify" onClick="classify()">CLASSIFY</button></div><br>
	Left confidence: <div id="left_conf">0%</div>	Right confidence: <div id="right_conf">0%</div>
...
def threshold_blue(pixel):
    if (pixel[0] < red_thresh and pixel[1] < green_thresh and pixel[2] > blue_thresh):
        pixel[0] = 0
        pixel[1] = 0
        pixel[2] = 255
...
def get_image(buf):
    ...
    arr = np.uint8(cm.jet(arr)*255)

    print(arr[12][16])

    for i, row in enumerate(arr):
        for j, pixel in enumerate(row):
            arr[i][j] = threshold_blue(pixel)

    # Save image into buffer
    im = Image.fromarray(arr).convert('RGB').rotate(180)#.transpose(Image.FLIP_LEFT_RIGHT)
...
class StreamingHandler(server.BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            ...
            print("Straight confidence level: " + str(label[2]*100) + "%")
            print("Tpose confidence level: " + str(label[3]*100) + "%")
            print("Unknown confidence level: " + str(label[4]*100) + "%")
            print()
